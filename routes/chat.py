    ...
    try:
        ...
        for log in chat_logs:
            ...
            if chat_id not in chats_dict:
                chats_dict[chat_id] = {
                    'id': chat_id,
                    'title': log['title'] or '',  # Use the title from database, fallback to empty string
                    'messages': [],
            ...
            # Update title if this log has a title (in case different messages have different titles)
            if log['title']:
...
async def update_chat_title(chat_id: str, data: dict):
    ...
    try:
        ...
        print(f"Verification - title after update: {verification['title'] if verification else 'NOT FOUND'}")  # Debug log
